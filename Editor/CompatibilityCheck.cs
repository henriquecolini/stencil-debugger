...
namespace StencilDebugger.Editor
{
    public class CompatibilityCheck : EditorWindow
    {
        ...
        [MenuItem("Window/Stencil Debugger/Compatibility")]
        ...
        private void CreateGUI()
        {
            ...
            var titleHeader = new Label("Stencil Debugger")
            {
                style =
                {
                    fontSize = 19,
            ...
            documentationLink.AddManipulator(new Clickable(() => Application.OpenURL("https://ameye.dev")));
        ...
        private CheckResult CheckUnityVersion()
        {
            ...
            if (unityVersion.StartsWith("6000.1"))
            {
                if (alpha || beta)
                {
                    return new CheckResult(ResultEnum.Warning, unityVersion, "Stencil Debugger is compatible with Unity 6.1. However, alpha/beta versions of Unity may be unstable.");
                }
                return new CheckResult(ResultEnum.Pass, unityVersion, "Stencil Debugger is compatible with Unity 6.1.");
            }
            if (unityVersion.StartsWith("6000"))
            {
                if (alpha || beta)
                {
                    return new CheckResult(ResultEnum.Warning, unityVersion, "Stencil Debugger is compatible with Unity 6.0. However, alpha/beta versions of Unity may be unstable.");
                }
                return new CheckResult(ResultEnum.Pass, unityVersion, "Stencil Debugger is compatible with Unity 6.0.");
            }
            if (unityVersion.StartsWith("2022.3"))
            {
                if (alpha || beta)
                {
                    return new CheckResult(ResultEnum.Warning, unityVersion, "Stencil Debugger is compatible with Unity 2022.3. However, alpha/beta versions of Unity may not be stable.");
                }
                return new CheckResult(ResultEnum.Warning, unityVersion,
                    "Stencil Debugger is compatible with Unity 2022.3. However, Unity no longer develops or improves the rendering path that does not use Render Graph API. Upgrade to Unity 6 to make use of the Render Graph API.");
            }
            if (unityVersion.StartsWith("2023"))
            {
                return new CheckResult(ResultEnum.Fail, unityVersion, $"Stencil Debugger is not compatible with Unity {unityVersion}. Please upgrade to Unity 6 which is the LTS version for the 2023 cycle.");
            }
            return new CheckResult(ResultEnum.Fail, unityVersion, $"Stencil Debugger is not compatible with Unity {unityVersion}.");
        }
        ...
        private CheckResult CheckURPVersion()
        {
            ...
            return new CheckResult(version.StartsWith("14") || version.StartsWith("17") ? ResultEnum.Pass : ResultEnum.Fail, version, "Stencil Debugger is compatible with URP 14 and URP 17.");
        }
        ...
        private CheckResult CheckActivePipeline()
        {
            ...
            {
                return new CheckResult(ResultEnum.Fail, "High Definition", "Stencil Debugger is not compatible with the High Definition Render Pipeline.");
            }

            if (type.Contains("UniversalRenderPipelineAsset"))
            {
                return new CheckResult(ResultEnum.Pass, "Universal", "Stencil Debugger is compatible with the Universal Render Pipeline.");
            }
            if (type.Contains("LightweightRenderPipelineAsset"))
            {
                return new CheckResult(ResultEnum.Fail, "Light Weight", "Stencil Debugger is not compatible with the Light Weight Render Pipeline");
            }
            return new CheckResult(ResultEnum.Fail, "Custom", "Stencil Debugger is not compatible with any Custom Render Pipeline.");
        }
        ...
        private CheckResult CheckActiveRenderer()
        {
            ...
            {
                UniversalRendererData => new CheckResult(result: ResultEnum.Pass, message: "Universal", description: "Stencil Debugger compatible with the Universal Renderer."),
        ...
        private CheckResult CheckRenderingPath()
        {
            ...
            switch (rendererData)
            {
                case UniversalRendererData data:
                {
                    ...
                    {
                        RenderingMode.Forward => new CheckResult(ResultEnum.Pass, "Forward", "Stencil Debugger is compatible with the Forward rendering path."),
                        RenderingMode.ForwardPlus => new CheckResult(ResultEnum.Pass, "Forward+", "Stencil Debugger is compatible with the Forward+ rendering path."),
                        RenderingMode.Deferred => new CheckResult(ResultEnum.Fail, "Deferred",
                            "Stencil Debugger is not compatible with the Deferred rendering path. You can change the active rendering path on your active Render Pipeline asset."),
